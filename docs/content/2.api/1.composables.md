...
To see examples of use you can refer also to the [playground app](https://github.com/atlasconsulting/nuxt-bedita/tree/main/playground).
...
- `user`{lang=ts} refers to the authenticated user data.
...
- `isLogged`{lang=ts} refers to info about user logged.
...
- `logout()`{lang=ts} is a logout helper function. It cleans session data.
...
- `optOut(username: string, password: string)`{lang=ts} performs a user opt-out deleting it from the BEdita.
